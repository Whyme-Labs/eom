"""Gemma-4-E4B SFT v4 — same recipe as v3, but train longer to crystallize
the EOM envelope schema.

v3 produced 5/5 unique structurally-similar JSON outputs but 0/5 were
schema-valid: the model learned "extract spans into JSON" but did not
internalize the `{version, lede, body, archive,...}` envelope. Stage 2's
Gemma-3-1B run needed 20 epochs to crystallize the same schema; mirror
that here.

Changes from v3:
- num_train_epochs: 5 → 20
- LoRA r=16, alpha=16 (slightly more capacity; alpha:r ratio held at 1:1)
"""
...
app = modal.App("eom-sft-gemma4-v4", image=image)
...
def train_gemma4_v4():
    ...
    args = TrainingArguments(
        output_dir="/output/eom-sft-out-gemma4-v4",
    ...
    out_path = "/output/val-generations-gemma4-v4.jsonl"
    with open(out_path, "w") as f:
        for g in gens:
            f.write(json.dumps(g) + "\n")
    print(f"saved {len(gens)} val generations -> {out_path}")

    out = "/output/eom-sft-adapter-gemma4-v4"
