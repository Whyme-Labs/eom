"""Gemma-4-E4B SFT v5 — push harder against E4B's strong instruction-tuned
prior that v4 couldn't crack.

v4 (20ep, r=16, α=16, lr=2e-4) produced 5/5 unique structurally-rich JSON
but 0/5 schema-valid: the model echoed prompt metadata keys
(`document_type`, `render_profile`, `source_text`) instead of producing
EOM `attention_budget` + `blocks` envelope. E4B's ~7B prior is heavier
than Gemma-3-1B's, so 20ep wasn't enough.

Changes from v4:
- num_train_epochs: 20 → 30 (more passes to override prior)
- LoRA r=32, alpha=32 (more capacity — optimizer + activations need
  A100-80GB; v4 already peaked at 40.01 GB on the 40 GB card)
- learning_rate: 2e-4 → 1e-4 (more stable convergence at higher capacity)
- GPU: A100-40GB → A100-80GB (headroom for bigger LoRA)
"""
...
app = modal.App("eom-sft-gemma4-v5", image=image)
...
def train_gemma4_v5():
    ...
    args = TrainingArguments(
        output_dir="/output/eom-sft-out-gemma4-v5",
    ...
    out = "/output/eom-sft-adapter-gemma4-v5"
